...
# Set to true to use partial_json_payload, false to use full json_payload
$usePartialPayload = $false
    ...
    try {
        ...
        while ($reader.Read()) {
            ...
            if (-not $rawJson -or $rawJson.Trim() -eq "") {
                Write-Host "⚠️ Skipping record — person_id '$personId' has NULL or empty JSON" -ForegroundColor Yellow
                continue
            }
...
<#
.SYNOPSIS
Compares two JSON-like objects and returns a diff of the changes.

.DESCRIPTION
Performs a deep recursive comparison between two PowerShell objects typically derived from JSON data.
It identifies differences between the current and previous objects, supporting nested structures,
dictionaries, and arrays. Only changed fields are included in the returned result.

Useful for detecting changes in structured payloads to support partial updates or selective processing.

.PARAMETER current
The current version of the JSON object (usually from the latest database or input source).

.PARAMETER previous
The previous version of the JSON object (typically from the last known or submitted state).

.OUTPUTS
Hashtable — a structure containing only the properties that have changed.
Returns an empty hashtable if no differences are found.

.EXAMPLE
$diff = Get-JsonDifferences -current $newJson -previous $oldJson
if ($diff.Count -gt 0) {
...
Version: 1.0  
Created: 2025-04-10  
Does not enforce schema validation — assumes input objects follow the same structure.
...
<#
.SYNOPSIS
Generates a partial JSON payload by comparing two structured objects.

.DESCRIPTION
Recursively compares the current and previous JSON objects to identify fields that have changed.
Returns a hashtable representing the minimal set of changes — i.e., a "diff" — which can be used
to construct partial payloads for efficient API submissions.

Always includes the `purge` flag where applicable, both at the top level and in nested structures,
to preserve correct API semantics. Also handles comparison of arrays and nested objects.

.PARAMETER Current
The current JSON object representing the most recent state of a record.

.PARAMETER Previous
The previous JSON object representing the last submitted or stored version of the record.

.OUTPUTS
Hashtable — a PowerShell object containing only the changed fields,
formatted in the same structure as the original JSON.

.EXAMPLE
$partial = Get-PartialJsonPayload -Current $newJson -Previous $oldJson

.NOTES
Author: D2I/RH  
Version: 1.0  
Created: 2025-04-10  
Useful for optimising payload sizes by sending only changed data to the API.
#>

function Get-PartialJsonPayload {
    param (
        [Parameter(Mandatory = $true)][object]$Current,
        [Parameter(Mandatory = $true)][object]$Previous
    )

    function Compare-Recursive($current, $previous) {
        $result = @{}

        foreach ($key in $current.PSObject.Properties.Name) {
            $currentVal = $current.$key
            $previousVal = $previous.$key

            # Always include purge flags
            if ($key -eq 'purge') {
                $result[$key] = $currentVal
                continue
            }

            if ($currentVal -is [PSCustomObject]) {
                if ($previousVal -isnot [PSCustomObject]) {
                    $result[$key] = $currentVal
                } else {
                    $nestedResult = Compare-Recursive $currentVal $previousVal
                    if ($nestedResult.Count -gt 0) {
                        # Preserve purge flag if exists in nested
                        if ($currentVal.PSObject.Properties.Name -contains 'purge') {
                            $nestedResult['purge'] = $currentVal.purge
                        }
                        $result[$key] = $nestedResult
                    }
                }
            } elseif ($currentVal -is [System.Collections.IEnumerable] -and
                      $currentVal -isnot [string]) {

                $arrayDiff = @()
                for ($i = 0; $i -lt $currentVal.Count; $i++) {
                    $cItem = $currentVal[$i]
                    $pItem = if ($i -lt $previousVal.Count) { $previousVal[$i] } else { $null }

                    if ($cItem -is [PSCustomObject] -and $pItem -is [PSCustomObject]) {
                        $diffItem = Compare-Recursive $cItem $pItem
                        if ($diffItem.Count -gt 0) {
                            # Preserve purge in array items if available
                            if ($cItem.PSObject.Properties.Name -contains 'purge') {
                                $diffItem['purge'] = $cItem.purge
                            }
                            $arrayDiff += $diffItem
                        }
                    } elseif ($cItem -ne $pItem) {
                        $arrayDiff += $cItem
                    }
                }
            ...
            } elseif ($currentVal -ne $previousVal) {
                $result[$key] = $currentVal
            }
        }
    ...
    return Compare-Recursive -current $Current -previous $Previous
}


<#
.SYNOPSIS
Retrieves the previous JSON payload for a given record from the database.

.DESCRIPTION
Connects to the specified SQL Server database and queries the `previous_json_payload`
field for the provided `person_id` in the given table. This payload is typically used
for comparison when generating partial payloads for API submission.

.PARAMETER connectionString
A valid SQL Server connection string for accessing the source database.

.PARAMETER tableName
The name of the database table containing the staging records with previous payloads.

.PARAMETER personId
The unique identifier of the child/person whose previous payload should be retrieved.

.OUTPUTS
String — the raw JSON string stored in the `previous_json_payload` column,
or `$null` if no match is found or an error occurs.

.EXAMPLE
$prevPayload = Get-PreviousJsonPayloadFromDb -connectionString $connStr -tableName "ssd_api_data_staging" -personId "abc123"

.NOTES
Author: D2I/RH  
Version: 1.0  
Created: 2025-04-10  
Useful for diffing old vs new payloads before generating and submitting partial updates.
#>
...
<#
.SYNOPSIS
Generates partial JSON payloads for all pending or errored records.

.DESCRIPTION
Compares the current and previous JSON payloads for each record with a status of 'pending' or 'error' in the specified table.
If differences are found, a new partial JSON payload is constructed containing only the changed fields.
Certain mandatory top-level fields (e.g. `la_child_id`, `mis_child_id`, `purge`, and `child_details`) are always included.
The resulting partial payload is saved to the `partial_json_payload` column in the staging table.

.PARAMETER connectionString
The SQL Server connection string used to connect to the source database.

.PARAMETER tableName
The name of the database table containing the API staging records.

.EXAMPLE
Generate-AllPartialPayloads -connectionString $connStr -tableName "ssd_api_data_staging"
...
Created: 2025-04-10  
Updated: 2025-04-11  
This function supports preprocessing of partial payloads to reduce unnecessary API submissions.
#>
...
function Generate-AllPartialPayloads {
    ...
    $query = "SELECT person_id, json_payload, previous_json_payload FROM $tableName WHERE submission_status IN ('pending', 'error');"
    $records = @()

    # Pull current + previous payloads
    try {
        $connection = New-Object System.Data.SqlClient.SqlConnection($connectionString)
        $connection.Open()
        $command = $connection.CreateCommand()
    ...
    foreach ($record in $records) {
        ...
        try {
            ...
            foreach ($prop in $diff.PSObject.Properties) {
                $partial[$prop.Name] = $prop.Value
            }

            # Always include required top-level fields
            foreach ($key in @("la_child_id", "mis_child_id", "purge")) {
                if ($current.PSObject.Properties.Match($key)) {
                    $partial[$key] = $current.$key
                }
            }

            if ($current.PSObject.Properties.Match("child_details")) {
                $partial["child_details"] = $current.child_details
            }

            $partialJson = $partial | ConvertTo-Json -Depth 20 -Compress
            $escaped = $partialJson -replace "'", "''"

            $updateQuery = @"
UPDATE $tableName
SET partial_json_payload = '$escaped'
WHERE person_id = '$personId';
"@
            Execute-NonQuerySql -connectionString $connectionString -query $updateQuery
        }
...
if ($usePartialPayload) {
    ...
    Generate-AllPartialPayloads -connectionString $connectionString -tableName $api_data_staging_table


    # Step 2: Re-fetch only records with valid partial JSONs
    Write-Host "🔁 Re-fetching filtered records with valid partial_json_payload..." -ForegroundColor Cyan
    $JsonArray = Get-PendingRecordsFromDb -connectionString $connectionString -tableName $api_data_staging_table -usePartialPayload:$true


    # If no partials exist after generation, skip entire submission
    if ($JsonArray.Count -eq 0) {
        Write-Host "✅ No changed records detected. Skipping API submission as partial mode is active." -ForegroundColor Green
        return
    }

    # # gives false positives.... to delete 110425
    ## Re-map partial_json into .json property for unified handling downstream
    #foreach ($record in $JsonArray) {
    #    try {
    #        $record | Add-Member -MemberType NoteProperty -Name json -Value ($record.partial_json_payload | ConvertFrom-Json -ErrorAction Stop) -Force
    #    } catch {
    #        Write-Host "⚠️ Skipping invalid partial JSON for person_id $($record.person_id)" -ForegroundColor Yellow
    #    }


    
} else {
    Write-Host "📦 Full payload mode active — using json_payload for all records" -ForegroundColor Green
}
...
for ($batchIndex = 0; $batchIndex -lt $totalBatches; $batchIndex++) {
    ...
    ### end partial json test




    # ensure batch is valid structure 
    # incl. if single record, we need to physically/coerce wrap it with [ ] before hitting api
