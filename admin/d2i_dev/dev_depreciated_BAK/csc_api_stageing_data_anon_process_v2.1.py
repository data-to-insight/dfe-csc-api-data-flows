...
try:
    ...
    cursor.execute("SELECT top 5 * FROM ssd_api_data_staging")
...
for row in rows:
    ...
    for worker in json_payload.get("social_workers", []):
        ...
        worker_start_date = worker.get("worker_start_date", "").split("T")[0]
        worker_end_date = worker.get("worker_end_date", "").split("T")[0]
        ...
        if worker_start_date and worker_end_date:
            worker["worker_start_date"], worker["worker_end_date"] = randomise_date_with_interval(worker_start_date, worker_end_date)
            anonymised_attributes.add("worker_start_date")
            anonymised_attributes.add("worker_end_date")

        # If start date exists, randomise but ensure it's after anon DoB
        elif worker_start_date:
            worker["worker_start_date"] = random_date_after(anon_dob, min_years=1, max_years=18) 
            anonymised_attributes.add("worker_start_date")
