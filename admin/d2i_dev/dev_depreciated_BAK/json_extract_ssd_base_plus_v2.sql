...
-- includes the starting point of fixed labels and disa list values. 
-- it does not yet address the BOOL flag formatting needed, and has the usac not yet as per spec. 
- but this runs
...
SELECT 
    ...
    (
        SELECT 
            ...
            cine.cine_referral_nfa AS [social_care_episodes.referral_no_further_action_flag],
            -- Nested CP Plans
            (
                SELECT 
            ...
            -- Nested Involvements for CIN Episode
            (
                SELECT 
                    pr.prof_staff_id AS [social_care_episodes.social_workers.worker_id],
                    CONVERT(VARCHAR(10), i.invo_involvement_start_date, 23) AS [social_care_episodes.social_workers.worker_start_date],
                    CASE 
                        WHEN i.invo_involvement_end_date IS NULL THEN NULL
                        ELSE CONVERT(VARCHAR(10), i.invo_involvement_end_date, 23)
                    END AS [social_care_episodes.social_workers.worker_end_date]
                ...
                WHERE i.invo_referral_id = cine.cine_referral_id
                  AND i.invo_professional_role_id = 'Allocated Case Worker' -- Filter
                FOR JSON PATH
            ) AS [social_care_episodes.social_workers]
