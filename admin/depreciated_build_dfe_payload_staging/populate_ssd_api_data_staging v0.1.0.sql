...
Script by default uses: 
FOR JSON PATH, JSON_QUERY, JSON_VALUE -> requires SQL Server 2016+ (2016 SP1 recommended)
STRING_AGG-> requires SQL Server 2017+
(The above will fail for earlier versions of SQL Server)
...
RAISERROR('== CSC API staging build: v%s ==', 10, 1, @VERSION) WITH NOWAIT;
...
IF OBJECT_ID(N'dbo.ssd_api_data_staging', N'U') IS NULL
BEGIN
    CREATE TABLE ssd_api_data_staging (
        id INT IDENTITY(1,1) PRIMARY KEY,           
        person_id NVARCHAR(48) NULL,               -- link value (_person_id)
        previous_json_payload NVARCHAR(MAX) NULL,  
...
WITH RawPayloads AS (
    -- staging data table record detail
    ...
    (
        ...
        CAST(0 AS bit) AS [purge],  -- root-level purge flag
...
-- ( -- enable to return key + empty [] on zero data
-- 	SELECT JSON_QUERY(
    -- 		'[' + 
    -- 		ISNULL(
        -- 			STRING_AGG('"' + disa_disability_code + '"', ',') 
        -- 			WITHIN GROUP (ORDER BY disa_disability_code),
        -- 			''
        -- 		) + 
        -- 		']'
        -- 	)
        -- 	FROM ssd_disability
        -- 	WHERE disa_person_id = p.pers_person_id
        -- ) AS [disabilities],
        disab.disabilities,
        (
...
(
    ...
    INNER JOIN ssd_cla_placement clap
...
OUTER APPLY (
    SELECT JSON_QUERY(
        '[' + STRING_AGG('"' + d.disa_disability_code + '"', ',') + ']'
    ) AS disabilities
...
-- insert JSON payload and hash into staging table
INSERT INTO ssd_api_data_staging (person_id, json_payload, current_hash)
SELECT 
person_id,
json_payload,
-- we only update the current_hash, ready for down-the-line comparison vs previous_
HASHBYTES('SHA2_256', CAST(json_payload AS NVARCHAR(MAX))) AS current_hash
FROM RawPayloads
WHERE json_payload IS NOT NULL; -- prevent blank rows mid-nesting fails



-- IMPORTANT
-- new setup requires populating prev_hash, as no previous exists
-- prime only rows that have never been primed
UPDATE s
SET s.previous_hash = s.current_hash
FROM dbo.ssd_api_data_staging AS s
WHERE s.previous_hash IS NULL;
...
IF OBJECT_ID('ssd_api_data_staging_anon', 'U') IS NOT NULL 
    DROP TABLE ssd_api_data_staging_anon;
