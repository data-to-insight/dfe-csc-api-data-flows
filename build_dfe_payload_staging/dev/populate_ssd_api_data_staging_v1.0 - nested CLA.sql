...
-- data loading for live ssd_api_data_staging

WITH RawPayloads AS (
    SELECT TOP (4000) -- TEST LIMITER
        ...
        -- payload starts here
            ...
            JSON_QUERY(
                ...
                SELECT 
					...
					-- ( -- enable to return key + empty [] structure on zero data
					-- 	SELECT JSON_QUERY(
					-- 		'[' + 
					-- 		ISNULL(
					...
					disab.disabilities, -- enable to omit [] structure on empty data
...
-- enable to omit [] structure on empty data
OUTER APPLY (
    SELECT JSON_QUERY(
